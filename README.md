...
**스토어프론트를 가질 것이라고 설계된 적 없는 ERP 앞에 B2B 커머스를 붙이는 작업.** 국내 B2B 화장품 원료 공급사 사례로, 고객이 원료를 드럼 단위로 주문하는 제조사들입니다. 주문 하나가 1톤을 넘어 해상 운송으로 나가고, 중요한 모든 것의 원본은 스토어프론트가 아니라 ERP에 있습니다.
...
| 재고의 원본은 스토어 | 재고 원본은 ERP, 스토어는 그 투영 |
| 고객은 사람 | 고객은 **ERP 거래처 코드를 가진 회사**이고, 그 코드 없는 주문은 쓸모가 없음 |
| 누구나 아무거나 구매 | 샘플은 구매자별 수량 제한을 결제 전에 강제 |
| 주문이 최종 상태 | 주문은 ERP의 **입력**이고, 그 이후 전부를 ERP가 소유 |

**서비스를 도메인이 아니라 트리거로 나눴습니다** — 웹훅 구동 / 구매 경로 요청 구동 / 장바구니 요청 구동 / 스케줄 구동. 장애 양상과 재시도 방식이 다른 것들은 한 프로세스를 공유하지 않아야 합니다.

| # | 케이스 | 핵심 문제 |
|---|---|---|
| 01 | ERP 주문 동기화 | ERP는 스토어프론트에 없는 거래처 코드를 요구함 |
| 02 | 중량 구간별 운임 견적 | 택배 기준으로 만들어진 체크아웃 안에서 해상 운임을 견적 |
| 03 | 샘플 구매 제한 | 플랫폼에 개념 자체가 없는 구매자별 상한을 강제 |
| 04 | 스케줄 재고 동기화 | 두 개의 재고 원장을 하루 두 번, 고객에게 거짓말하지 않으면서 맞추기 |

**공통 실천**

- 모든 외부 호출은 백오프 재시도(`tenacity`), 모든 실패는 **DB에 기록**. "주문이 ERP에 안 들어갔다는데요"는 3주 전 로그를 grep해서 답할 질문이 아니라 테이블을 조회해서 답할 질문입니다.
- ERP 세션은 스스로 갱신 — ERP가 짧은 세션을 발급하고 계정을 지역 존에 분산 배치하는데, 클라이언트가 존 판별과 세션 갱신을 알아서 처리해 호출부는 둘의 존재를 몰라도 됩니다.
- 설정은 타입 검증 후 부팅 — 필수 값에 기본값을 주지 않아서, 누락 시 첫 웹훅이 들어오는 새벽 2시가 아니라 부팅 시점에 필드명과 함께 실패합니다.
- 웹훅은 파싱보다 HMAC 검증이 먼저.
